...
void HVH::FakeMove(CUserCmd* cmd)
{

	if (g_input.GetKeyState(g_menu.main.movement.fakewalk.get())) // we dont want to be fakewalking and doing this too, very bad things will happen.
		return;

	vec3_t velocity = g_cl.m_local->m_vecVelocity(); // get our current velocity.

	if (velocity.length_2d() < 0.1f) // if we're not moving why would we try to do this?
		return;
	...
	const int32_t max_ticks = g_cl.m_max_lag;
	const int32_t chocked = g_cl.m_lag;
	int32_t ticks_left = max_ticks - chocked;
	if (ticks_to_stop < ticks_left && !*g_cl.m_packet) // this will be our final fakelag "cycle" meaning this is the last time we'll be able to update
	{														// update our real before stopping, without fucking up our fakelag value ofc.
		g_cl.m_view_angles.y += 110;
	}
}

void HVH::DistortionAntiAim(CUserCmd* cmd) //the new era of anti-aim by kawaii.
{
...
bool IsPressingMovementKeys(CUserCmd* cmd)
{
	...
	if (cmd->m_buttons & IN_FORWARD ||
		cmd->m_buttons & IN_BACK || cmd->m_buttons & IN_LEFT || cmd->m_buttons & IN_RIGHT ||
