...
void Hooks::OnRenderStart() {
	...
	if (g_menu.main.skins.fov.get()) {
		if (g_cl.m_local && g_cl.m_local->m_bIsScoped()) {
			if (g_menu.main.skins.fov_scoped.get()) {
			...
			else if (!g_menu.main.skins.fov_scoped.get())
				if (g_cl.m_local->GetActiveWeapon()->m_zoomLevel() != 2) {
					g_csgo.m_view_render->m_view.m_fov = 90.f - ((g_menu.main.skins.fov_amt.get() / 100) * 80 / 4);
				}
				else {
					g_csgo.m_view_render->m_view.m_fov = 90.f - ((g_menu.main.skins.fov_amt.get() / 100) * 80 / 4);
				}
		}
		else g_csgo.m_view_render->m_view.m_fov = g_menu.main.skins.fov_amt.get();
	}

	if (g_menu.main.skins.viewmodel_fov.get())
