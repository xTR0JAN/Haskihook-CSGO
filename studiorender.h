...
public:
	__forceinline bool IsForcedMaterialOverriden() {
		if (!m_pForcedMaterial)
			return m_overrideType == OverrideType::DepthWrite || m_overrideType == OverrideType::SSaoDepthWrite;
		return std::string_view{ m_pForcedMaterial->GetName() }._Starts_with(XOR("dev/glow"));
	}
